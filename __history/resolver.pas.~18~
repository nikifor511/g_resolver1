...
begin
  try
    ...
    myIdHTTP.Get('http://resolver/problem_chat/' + IntToStr(problem_id),Stream);
...
begin
  resultHTTPGet := myIdHTTP.Get('http://resolver/?oper=__log&name=' + fio + '&pass=' + pass);
...
begin
  resultHTTPGet := myIdHTTP.Get('http://resolver/?oper=___lo&name=' + self.FIO);
  if resultHTTPGet <> '0' then begin
    problems.Clear;
...
begin
  Result := StrToInt(myIdHTTP.Get('http://resolver/?oper=___cp&name=' + self.FIO +
